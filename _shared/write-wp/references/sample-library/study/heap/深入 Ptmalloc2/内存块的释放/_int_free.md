---
title: _int_free
date: 2025-04-27T21:24:07+08:00
lastmod: 2025-04-27T22:02:16+08:00
---

# _int_free

函数在初始时刻定义了一系列的变量，并且得到了用户 所想要释放的 chunk 的大小
...
如果上述的简单检查都合格，则会判断当前 bin 是否在 Fast Bin 范围内。

如果在，则插入至 Fast Bin 头部，即成为对应 Fast Bin 链表的 **第一个 Free Chunk。**
...
# 对 非mmap 的空闲 chunk 进行 合并

 **⚠️只有不是 Fast Bin 的情况下才会触发 unlink**

‍

首先，我们先解释一下为什么会对 Chunk 进行合并。

这是为了避免 heap 中存在太多零碎的内存块，合并后可以用来应对更大的内存块请求。
...
## 下一块不是 Top Chunk - 向前合并 - 合并高地址的 chunk

需要我们注意的是，如果下一块并不是 Top Chunk，则合并高地址的 chunk，并向合并后的 chunk 放入 Unsorted Bin 中。
